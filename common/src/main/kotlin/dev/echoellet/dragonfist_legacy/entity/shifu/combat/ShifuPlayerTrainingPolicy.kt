...
import dev.echoellet.dragonfist_legacy.entity.shifu.combat.ShifuPlayerTrainingPolicy.Companion.PLAYER_SAFE_HEALTH
import dev.echoellet.dragonfist_legacy.entity.shifu.combat.ShifuPlayerTrainingPolicy.Companion.clampDamageIfNeeded
...
class ShifuPlayerTrainingPolicy(private val entity: ShifuEntity) {
    companion object {
         ...
         *
         * Notes:
         * - Attack damage can be unpredictable and sometimes exceeds this threshold.
         ...
         * - To work around this issue, the event in [clampDamageIfNeeded] forces the player's health
        ...
        /**
         * The health value used by [clampDamageIfNeeded] to clamp incoming damage from Shifu.
         *
         ...
         * - With [clampDamageIfNeeded] in place, Shifu’s attacks will never directly
         ...
         *
         * This depends on the mod loader to work or a mixin on [LivingEntity.hurt], as there is no reliable vanilla
         * Minecraft equivalent for safely intercepting pre-damage logic.
         *
         * @see [PLAYER_SAFE_HEALTH]
         */
        fun clampDamageIfNeeded(
            target: LivingEntity,
            attacker: Entity?,
            incomingDamage: Float
        ): Optional<Float> {
            if (attacker !is ShifuEntity || target !is Player) return Optional.empty()

            val currentHealth = target.health
            val minHealth = PLAYER_SAFE_HEALTH

            val dropsBelowMinHealth = currentHealth - incomingDamage < minHealth
            if (!dropsBelowMinHealth) return Optional.empty()
    ...
    private fun stopTraining(player: Player, message: Component) {
        ...
        if (!player.isAlive) {
            /**
             * Shifu should never be able to kill the player because of [clampDamageIfNeeded].
